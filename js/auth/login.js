...
function performLogin() {
    ...
    if (!selectedRole) {
        errorDiv.textContent = 'Please select a role (Instructor or Admin)';
        return;
    }
    ...
    if (user.role !== selectedRole) {
        errorDiv.textContent = `This account is registered as "${user.role}", not "${selectedRole}". Please select the correct role.`;
        return;
    }
