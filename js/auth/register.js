...
function selectRegRole(role) {
    // SECURITY: Block admin role selection
    if (role === 'admin') {
        showNotification('Admin registration is disabled. Only instructors can create accounts.', 'error');
        return;
    }

    selectedRegRole = role;
    document.querySelectorAll('#registerMode .role-btn').forEach(btn => btn.classList.remove('selected'));
    document.querySelector(`#registerMode .role-btn.${role}`).classList.add('selected');
...
function performRegistration() {
    ...
    if (selectedRegRole === 'admin') {
        messageEl.textContent = 'Admin accounts cannot be created through registration. Please contact system administrator.';
